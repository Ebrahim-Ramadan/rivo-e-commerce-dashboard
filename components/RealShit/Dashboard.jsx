          ...
          <div key={order.id} className={`flex flex-col md:flex-row gap-6 border border-white/20 p-4 rounded-lg`}>
            <div className="rounded-lg shadow-md flex-grow">
              ...
              {order.items?.map((item, index) => (
                <a key={index} className="bg-white/10 hover:bg-white/20 flex items-center p-4 mb-2 rounded-lg justify-between w-full" target='_blank' href={`https://e-commerce-myass.vercel.app/frame/${item.id}?type=${item.type}&size=${item.size}&color=${item.color}`}>
